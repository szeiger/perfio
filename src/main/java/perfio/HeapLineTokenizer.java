...
abstract sealed class HeapLineTokenizer extends LineTokenizer implements CloseableView permits HeapScalarLineTokenizer, HeapVectorizedLineTokenizer {
  ...
  protected final HeapBufferedInput bin;

  protected HeapLineTokenizer(HeapBufferedInput parentBin, byte eolChar, byte preEolChar) throws IOException {
  ...
  protected String emit(int start, int lfpos) {
