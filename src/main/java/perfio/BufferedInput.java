...
public abstract class BufferedInput implements Closeable {
   ...
   * Same as {@code of(in, order, 32768)}
   *
   * @param in                InputStream from which to read data.
  ...
  /**
   * Read data from an {@link InputStream} using the default byte order.
   * Same as {@code of(in, ByteOrder.BIG_ENDIAN, initialBufferSize)}
   *
   * @param in                InputStream from which to read data.
  ...
  /**
   * Read data from an {@link InputStream}.
   *
   * @param in                InputStream from which to read data.
   * @param order             Byte order used for reading multi-byte values. Default: Big endian.
   * @param initialBufferSize Initial size of the buffer in bytes. It is automatically extended if necessary. This also
  ...
  /** Same as {@code ofArray(buf, 0, buf.length, ByteOrder.BIG_ENDIAN)}
  ...
  /** Read data from a {@link MemorySegment}. Use {@link MemorySegment#asSlice(long, long)} for reading only part of a segment.
   *
   * @param ms             Segment from which to read.
   * @param closeable      An optional {@link Closeable} object to close when the BufferedInput is closed, otherwise null.
  ...
  /** Same as {@code ofMemorySegment(ms, null, ByteOrder.BIG_ENDIAN)}
  ...
  /** Read data from a {@link ByteBuffer}. The BufferedInput will start at the current position and end at the current
  ...
  /** Same as {@code ofMappedFile(file, ByteOrder.BIG_ENDIAN)}
  ...
  /** Same as {@code delimitedView(limit, false) }
  ...
  /**
   * Create a vectorized or scalar {@link LineTokenizer} depending on JVM and hardware support.
   *
   * The LineTokenizer is treated like a view, i.e. no other access of this BufferedReader (except closing it) is
   * allowed until the LineTokenizer is closed with {@link LineTokenizer#close()} (thus closing this BufferedInput as well)
   * or {@link LineTokenizer#end()} (to keep this BufferedInput open for further reading).
   *
